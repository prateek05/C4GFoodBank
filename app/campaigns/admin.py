...
class ExportCsvMixin:
    def export_as_csv(self, request, queryset):
        ...
        writer = csv.writer(
            response, delimiter="|", quotechar="`", quoting=csv.QUOTE_NONE
        )
        field_names = []
        for field in fields:
        ...
        if request.method == "POST":
            ...
            for row in reader:
                Campaign.objects.update_or_create(
                    ...
                    defaults={k: v for k, v in row.items() if v and k != "campaign_id"},
                )
        ...
        if request.method == "POST":
            ...
            for row in reader:
                Question.objects.update_or_create(
                    ...
                    defaults={k: v for k, v in row.items() if v and k != "question_id"},
                )
            self.message_user(request, "Your csv file has been imported")
            return redirect("..")
        form = CsvImportForm()
        ...
        if request.method == "POST":
            ...
            for row in reader:
                AnswerChoice.objects.update_or_create(
                    ...
                    defaults={k: v for k, v in row.items() if v and k != "answer_id"},
                )
            self.message_user(request, "Your csv file has been imported")
            return redirect("..")
        form = CsvImportForm()
