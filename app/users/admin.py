...
from campaigns.admin import ExportCsvMixin, CsvImportForm
...
class CampaignUserAdmin(admin.ModelAdmin,ExportCsvMixin):
    ...
    def import_csv(self, request):
        if request.method == "POST":
            ...
            for row in reader:
                CampaignUser.objects.update_or_create(user_id=row["user_id"], defaults={k: v for k, v in row.items() if v and k != "user_id" })
...
class SurverySiteAdmin(admin.ModelAdmin,ExportCsvMixin):
    ...
    def import_csv(self, request):
        if request.method == "POST":
            ...
            reader = csv.DictReader(StringIO(uploaded_file),delimiter='|',quotechar= '`', quoting=csv.QUOTE_NONE)
            for row in reader:
                AgencySite.objects.update_or_create(site_id=row["site_id"], defaults={k: v for k, v in row.items() if v and k != "site_id" })
            self.message_user(request, "Your csv file has been imported")
            return redirect("..")
        form = CsvImportForm()
        payload = {"form": form}
